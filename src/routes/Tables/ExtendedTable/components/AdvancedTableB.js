...
const generateRow = id => ({
  ...
  earningsCurrencyIcon: randomArray([
    <i className="fa fa-fw fa-euro text-muted" key="cur_eur" />,
    <i className="fa fa-fw fa-dollar text-muted" key="cur_usd" />,
  ]),
...
const sortCaret = order => {
  if (!order) return <i className="fa fa-fw fa-sort text-muted" />;
  if (order) return <i className={`fa fa-fw text-muted fa-sort-${order}`} />;
};
